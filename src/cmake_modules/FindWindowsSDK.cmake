...
#
# Pass "COMPONENTS tools" to ignore Visual Studio version checks: in case you just want the tool binaries to run, rather
# than the libraries and headers for compiling.
#
# Variables: WINDOWSSDK_FOUND - if any version of the windows or platform SDK was found that is usable with the current
# version of visual studio WINDOWSSDK_LATEST_DIR WINDOWSSDK_LATEST_NAME WINDOWSSDK_FOUND_PREFERENCE - if we found an
# entry indicating a "preferred" SDK listed for this visual studio version WINDOWSSDK_PREFERRED_DIR
# WINDOWSSDK_PREFERRED_NAME
#
# WINDOWSSDK_DIRS - contains no duplicates, ordered most recent first. WINDOWSSDK_PREFERRED_FIRST_DIRS - contains no
# duplicates, ordered with preferred first, followed by the rest in descending recency
#
# Functions: windowssdk_name_lookup(<directory> <output variable>) - Find the name corresponding with the SDK directory
# you pass in, or NOTFOUND if not recognized. Your directory must be one of WINDOWSSDK_DIRS for this to work.
#
# windowssdk_build_lookup(<directory> <output variable>) - Find the build version number corresponding with the SDK
# directory you pass in, or NOTFOUND if not recognized. Your directory must be one of WINDOWSSDK_DIRS for this to work.
#
# get_windowssdk_from_component(<file or dir> <output variable>) - Given a library or include dir, find the Windows SDK
# root dir corresponding to it, or NOTFOUND if unrecognized.
#
# get_windowssdk_library_dirs(<directory> <output variable>) - Find the architecture-appropriate library directories
# corresponding to the SDK directory you pass in (or NOTFOUND if none)
#
# get_windowssdk_library_dirs_multiple(<output variable> <directory> ...) - Find the architecture-appropriate library
# directories corresponding to the SDK directories you pass in, in order, skipping those not found. NOTFOUND if none at
# all. Good for passing WINDOWSSDK_DIRS or WINDOWSSDK_DIRS to if you really just want a file and don't care where from.
#
# get_windowssdk_include_dirs(<directory> <output variable>) - Find the include directories corresponding to the SDK
# directory you pass in (or NOTFOUND if none)
#
# get_windowssdk_include_dirs_multiple(<output variable> <directory> ...) - Find the include directories corresponding
# to the SDK directories you pass in, in order, skipping those not found. NOTFOUND if none at all. Good for passing
# WINDOWSSDK_DIRS or WINDOWSSDK_DIRS to if you really just want a file and don't care where from.
#
# Requires these CMake modules: FindPackageHandleStandardArgs (known included with CMake >=2.6.2)
#
# Original Author: 2012 Ryan Pavlik <rpavlik@iastate.edu> <abiryan@ryand.net> http://academic.cleardefinition.com Iowa
# State University HCI Graduate Program/VRAC
#
# Copyright Iowa State University 2012. Distributed under the Boost Software License, Version 1.0. (See accompanying
...
# See https://developer.microsoft.com/en-us/windows/downloads/sdk-archive - although version numbers listed on that page
# don't necessarily match the directory used by the installer.
...
# Appends to the three main pre-output lists used only if the path exists and is not already in the list.
...
function(_winsdk_conditional_append_preferred _info _path)
  ...
  set(_win_sdk_preferred_sdk_dirs
      "${_win_sdk_dirs}"
      CACHE INTERNAL "" FORCE)
...
# Given a version like v7.0A, looks for an SDK in the registry under "Microsoft SDKs". If the given version might be in
# both HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SDKs\\Windows and
# HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows Kits\\Installed Roots aka "Windows Kits", use this macro first, since
# these registry keys usually have more information.
#
...
function(_winsdk_check_microsoft_sdks_registry _winsdkver)
  ...
  set(_build ${ARGN})
  # See if the registry holds a Microsoft-mutilated, err, designated, product name (just using get_filename_component to
  ...
  # try for a version to augment our name (just using get_filename_component to execute the registry lookup)
  ...
  if(NOT "${_sdkver}" MATCHES "registry" AND NOT MATCHES)
    ...
    if(NOT "${_sdkver}" MATCHES "\\.\\.")
      # and it's not an invalid one with two dots in it: use to override the default build
      set(_build ${_sdkver})
...
# Given a name for identification purposes, the build number, and a key (technically a "value name") corresponding to a
# Windows SDK packaged as a "Windows Kit", look for it in HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows
# Kits\\Installed Roots Note that the key or "value name" tends to be something weird like KitsRoot81 - no easy way to
# predict, just have to observe them in the wild. Doesn't hurt to also try _winsdk_check_microsoft_sdks_registry for
# these: sometimes you get keys in both parts of the registry (in the wow64 portion especially), and the non-"Windows
# Kits" location is often more descriptive.
function(_winsdk_check_windows_kits_registry _winkit_name _winkit_build _winkit_key)
  get_filename_component(
    _sdkdir "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows Kits\\Installed Roots;${_winkit_key}]" ABSOLUTE)
  _winsdk_conditional_append("${_winkit_name}" "${_winkit_build}" "${_sdkdir}")
endfunction()

# Given a name for identification purposes and the build number corresponding to a Windows 10 SDK packaged as a "Windows
# Kit", look for it in HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows Kits\\Installed Roots Doesn't hurt to also try
# _winsdk_check_microsoft_sdks_registry for these: sometimes you get keys in both parts of the registry (in the wow64
# portion especially), and the non-"Windows Kits" location is often more descriptive.
function(_winsdk_check_win10_kits _winkit_build)
  get_filename_component(_sdkdir "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows Kits\\Installed Roots;KitsRoot10]"
                         ABSOLUTE)
...
# Given a name for indentification purposes, the build number, and the associated package GUID, look in the registry
# under both HKLM and HKCU in \\SOFTWARE\\Microsoft\\MicrosoftSDK\\InstalledSDKs\\ for that guid and the SDK it points
# to.
...
if(MSVC AND NOT _WINDOWSSDK_IGNOREMSVC)
  ...
  elseif("${CMAKE_VS_PLATFORM_TOOLSET}" STREQUAL "v100" OR "${CMAKE_VS_PLATFORM_TOOLSET}" STREQUAL "v90")
    ...
    # OK, we're VC11 or newer and not using a backlevel or XP-compatible toolset. These versions have no XP (and
    # possibly Vista pre-SP1) support
    set(_winsdk_vistaonly_ok ON)
    if(_WINDOWSSDK_ANNOUNCE AND NOT _WINDOWSSDK_VISTAONLY_PESTERED)
      set(_WINDOWSSDK_VISTAONLY_PESTERED
          ON
          CACHE INTERNAL "" FORCE)
      message(
        STATUS
...
#
# MSVC version checks - keeps messy conditionals in one place (messy because of _WINDOWSSDK_IGNOREMSVC)
#
...
if(_winsdk_msvc_greater_1310) # Newer than VS .NET/VS Toolkit 2003
  ...
  if(_winsdk_msvc_less_1600)
    ...
    get_filename_component(
      _sdkdir "[HKEY_CURRENT_USER\\Software\\Microsoft\\Microsoft SDKs\\Windows;CurrentInstallFolder]" ABSOLUTE)
    _winsdk_conditional_append_preferred("Per-user current Windows SDK" "${_sdkdir}")

    # System-wide current Windows SDK for VS2005/2008
    get_filename_component(
      _sdkdir "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SDKs\\Windows;CurrentInstallFolder]" ABSOLUTE)
  ...
  if(_winsdk_vistaonly_ok AND _winsdk_msvc_not_less_1800)
    ...
    # Windows Software Development Kit (SDK) for Windows 10 Several different versions living in the same directory - if
    # nothing else we can assume RTM (10240)
  ...
  # Included in Visual Studio 2013 Includes the v120_xp toolset
  _winsdk_check_microsoft_sdks_registry(v8.1A 8.1.51636)

  if(_winsdk_vistaonly_ok AND _winsdk_msvc_not_less_1800)
    # Windows Software Development Kit (SDK) for Windows 8.1 http://msdn.microsoft.com/en-gb/windows/desktop/bg162891
  ...
  if(_winsdk_vistaonly_ok)
    ...
    # Microsoft Windows SDK for Windows 8 and .NET Framework 4.5 This is the first version to also include the DirectX
    # SDK http://msdn.microsoft.com/en-US/windows/desktop/hh852363.aspx
  ...
  # Included with VS 2012 Update 1 or later Introduces v110_xp toolset
  ...
  # Windows SDK for Windows 7 and .NET Framework 3.5 SP1 Works with VC9
  # http://www.microsoft.com/en-us/download/details.aspx?id=18950
  _winsdk_check_microsoft_sdks_registry(v7.0 6.1.7600.16385)

  # Two versions call themselves "v6.1": Older: Windows Vista Update & .NET 3.0 SDK
  # http://www.microsoft.com/en-us/download/details.aspx?id=14477

  # Newer: Windows Server 2008 & .NET 3.5 SDK may have broken VS9SP1? they recommend v7.0 instead, or a KB...
...
if(_winsdk_msvc_greater_1200)
  _winsdk_check_platformsdk_registry("Microsoft Platform SDK for Windows Server 2003 R2" "5.2.3790.2075.51"
                                     "D2FF9F89-8AA2-4373-8A31-C838BF4DBBE1")
  _winsdk_check_platformsdk_registry("Microsoft Platform SDK for Windows Server 2003 SP1" "5.2.3790.1830.15"
...
find_package_handle_standard_args(WindowsSDK "No compatible version of the Windows SDK or Platform SDK found."
                                  WINDOWSSDK_DIRS)
...
if(WINDOWSSDK_FOUND)
  ...
  function(get_windowssdk_library_dirs _winsdk_dir _var)
    ...
    foreach(
      ...
      list(APPEND _suffixes "lib/${_ver}/${_winsdk_arch}" "lib/${_ver}/um/${_winsdk_arch8}"
    ...
    foreach(_mode umdf kmdf)
      file(
        GLOB _wdfdirs
        RELATIVE "${_winsdk_dir}"
        "${_winsdk_dir}/lib/wdf/${_mode}/${_winsdk_arch8}/*")
      if(_wdfdirs)
