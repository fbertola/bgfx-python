...
//									);
	...
	int compileShader(Options& options)
	{
	    ...
	    std::string dir;
		{
			bx::FilePath fp(options.inputFilePath.c_str());
		...
		bx::FileReader reader;
		if (!bx::open(&reader, options.inputFilePath.c_str()) )
		{
		...
		else
		{
			...
			if ('c' != options.shaderType)
			{
				...
				const char* varyingdef = defaultVarying.c_str();
			...
			if (!bx::open(writer, options.outputFilePath.c_str()) )
			{
				bx::printf("Unable to open output file '%s'.", options.outputFilePath.c_str());
		...
		bx::remove(options.outputFilePath.c_str());
